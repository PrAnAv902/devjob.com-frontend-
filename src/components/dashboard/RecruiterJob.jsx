...
const RecruiterJob = () => {
    ...
    async function ediJobDataCall(){
      dispatch(setLoading(true));
      try{
         const res = await apiConnector("PUT",recruiterEndpoints.GET_EDIT_JOB_DATA_API,{jobId:id},{'Authorization': `Bearer ${token}`});
         if(!res?.data.responseCode){
           throw new Error(res.data)
         }
         setEditData(res.data.job);
      }
      catch(error){
        if(error.response.data.responseCode == 401){
          localStorage.removeItem("token");
    ...
    async function editJob(newData){
      dispatch(setLoading(true));
      try{
        const res = await apiConnector("POST",recruiterEndpoints.EDIT_JOB_API + "/" + `${id}`,newData,{'Authorization': `Bearer ${token}`});    
        if(!res?.data.responseCode){
            throw new Error(res.data)
        }
        localStorage.removeItem("user");
  ...
  useEffect(()=>{
    ...
    setValue("workingMode",editData?.workingMode);
    ...
    setRichText(editData?.description);
  },[editData])
